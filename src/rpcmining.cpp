...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("StakeReadyE-94", ((uint64_t)pwalletMain->GetStakeWeight(*pwalletMain, STAKE_NORMAL))/730));
...
// E-94: Return average network hashes per second based on last number of blocks.
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Element-94 is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Element-94 is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Element-94 is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Element-94 is downloading blocks...");
